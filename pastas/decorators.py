...
def set_parameter(function):
    ...
    def _set_parameter(self, name, value, **kwargs):
        if name not in self.parameters.index:
            logger.error(f"Parameter name {name} does not exist, please "
                         f"choose from {self.parameters.index}")
        else:
...
def get_stressmodel(function):
    ...
    def _get_stressmodel(self, name, **kwargs):
        if name not in self.stressmodels.keys():
            logger.error(f"The stressmodel name you provided is not in the "
                         f"stressmodels dict. Please select from the "
                         f"following list: {self.stressmodels.keys()}")
        else:
