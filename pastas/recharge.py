...
class LinearFlex(RechargeBase):
    """Adjusted model for precipitation excess according.
    ...
    Where f is the evaporation factor (like in Linear) and f2 is a factor
    detemined by the state of the unsaturated zone. This state is calculated by
    a bucket with a level s, with a maximum capcity of smax:
    ...
    def get_init_parameters(self, name="recharge"):
        parameters = DataFrame(
            columns=["initial", "pmin", "pmax", "vary", "name"])
        parameters.loc[name + "_f"] = (-1.0, -2.0, 0.0, True, name)
        parameters.loc[name + "_smax"] = (30., 1., 300.0, True, name)
        parameters.loc[name + "_smin"] = (1.0, 0., 300.0, False, name)
        return parameters
    ...
    def simulate(self, prec, evap, p, **kwargs):
        ...
        ----------
        prec, evap: array_like
            array with the precipitation and potential evaporation values.
            These arrays must be of the same length and at the same time steps.
        p: array_like
            array_like object with the values as floats representing the
            model parameters.
        ...
        """
        r = self.get_recharge(prec, evap, f=p[0], smax=p[1], smin=p[2])[0]
        return r

    @staticmethod
    @njit
    def get_recharge(prec, evap, f, smax, smin, dt=1.0):
        """
        Internal method used for the recharge calculation. If Numba is
        available, this method is significantly faster.
        ...
        n = prec.size
        # Create empty arrays to store the fluxes and states
        s = zeros(n, dtype=float64)  # Storage State
        s[0] = 0.5 * smax  # Set the initial system state to half-full
        ea = zeros(n, dtype=float64)  # Actual evaporation Flux
        pe = zeros(n, dtype=float64)  # Effective precipitation Flux
        r = zeros(n, dtype=float64)  # Recharge Flux

        for t in range(n - 1):
            f2 = max((s[t] - smin) / (smax - smin), 0.0)
            ea[t] = f * f2 * evap[t]
            pe[t] = prec[t]
            r[t] = f2 * pe[t] + ea[t]
            # Calculate state of the root zone storage
            s[t + 1] = min(s[t] + dt * (pe[t] + ea[t]), smax)

        return r, ea, pe, s

    def get_water_balance(self, prec, evap, p, dt=1.0, **kwargs):
        r, ea, pe, s = self.get_recharge(prec, evap, srmax=p[0],
                                         lp=p[1], ks=p[2],
                                         gamma=p[3], simax=p[4],
                                         kv=p[5], dt=dt)
