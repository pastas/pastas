...
class TrackSolve:
    ...
    def __init__(
        ...
        # calculate EVP
        self.evp = np.array([evp(oseries=self.obs, res=res)])
    ...
    def track_solve(self, p: ArrayLike | None = None, **kwargs) -> None:
        ...
        if "params" in kwargs:
            deprecate_args_or_kwargs(
                name="params",
                version="2.3.0",
                reason="Please use `p` instead of `params`.",
            )
            if p is None:
                p = kwargs.pop("params")
            else:
        ...
        if kwargs:
            raise TypeError(
                f"track_solve() got unexpected keyword argument '{next(iter(kwargs))}'"
            )

        if p is None:
            raise TypeError("track_solve() missing required argument: 'p'")
        ...
        # recalculate EVP
        self.evp = np.r_[self.evp, evp(oseries=self.obs, res=r_res)]
    ...
    def _noise(self, p: ArrayLike) -> ArrayLike:
        """get noise.
    ...
    def _residuals(self, p: ArrayLike) -> ArrayLike:
        """calculate residuals.
    ...
    def plot_track_solve(self, p: ArrayLike | None = None, **kwargs) -> None:
        ...
        if kwargs:
            raise TypeError(
                "plot_track_solve() got unexpected keyword argument "
                f"'{next(iter(kwargs))}'"
            )

        if p is None:
            raise TypeError("plot_track_solve() missing required argument: 'p'")
